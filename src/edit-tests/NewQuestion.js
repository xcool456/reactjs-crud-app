...
class NewQuestion extends React.Component {
    ...
    createItem() {
        if (
            this.props.location &&
            new URLSearchParams(this.props.location).get('id')
        ) {
            const db = new URLSearchParams(this.props.location).get('db');
    ...
    componentDidMount() {
        if (this.props.location) {
            this.setState({
                formControls: {
                    question: new URLSearchParams(this.props.location).get(
                        'question'
                    ),
                    code:
                        new URLSearchParams(this.props.location).get(
                            'code'
                        ) == 'null'
                            ? ''
                            : new URLSearchParams(this.props.location).get(
                    ...
                    answers: {
                        answer0: new URLSearchParams(this.props.location).get(
                            'answer0'
                        ),
                        answer1: new URLSearchParams(this.props.location).get(
                            'answer1'
                        ),
                        answer2: new URLSearchParams(this.props.location).get(
                            'answer2'
                        ),
                        answer3: new URLSearchParams(this.props.location).get(
                            'answer3'
                        ),
                        answer4: new URLSearchParams(this.props.location).get(
                            'answer4'
                        ),
                    },
        ...
        return (
            <div className="container">
                ...
                <div className="row">
                    <div className="col-12 font-weight-bold">
                        <label>
                            Введите номер правильного ответа(отсчет с 0):{' '}
                        </label>
                        <input
